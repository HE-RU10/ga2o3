...
def uncut_Ga2O3(filein, fileout, binWidth, limit_length, outType='xyz', element={1:'W'}, debug=False):
    """
            ...
            if bin_len_max * binWidth < limit_length:
                ...
                if outType == 'xyz':
                    ...
                    with open(fileout, 'w') as out:
                        ...
                        for i in range(len(final_coord)):
                            propid = prop[i]
                            out.write("%d %.5f %.5f %.5f %s\n" % \
                                      (propid, final_coord[i][0], final_coord[i][1], final_coord[i][2], element[propid]))
                                
                elif outType == 'lammps/data':
                    data.particles.positions = final_coord
                    export_file(data, fileout, outType)
                    
                else:
                    raise ValueError("The code does not support %s output." % outType)
...
if __name__ == '__main__':
    ...
    uncut_Ga2O3(loc_Ga2O3, locout_Ga2O3, 1, 3.1652, outType='xyz', element=element_Ga2O3)
